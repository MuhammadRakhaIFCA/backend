...
export class UploadService {
    ...
    async extractWordCoordinates(filePath: string, targetWord: string) {
        ...
        const { data } = await Tesseract.recognize(filePath, 'eng', { logger: console.log });
        ...
        for (const item of textContent.items) {
            ...
            if (currentY === null || Math.abs(currentY - y) <= 2) {
                ...
                if (
                    ...
                    x - (currentLine[currentLine.length - 1].x + currentLine[currentLine.length - 1].width) <= 3
                ) {
                    // Add to the current line
                    currentLine.push({ text: textString, x, width, fontName, textHeight });
                ...
                // Close the current line and start a new one
                if (currentLine.length > 0) groupedLines.push({ y: currentY, line: currentLine });
                currentLine = [{ text: textString, x, width, fontName, textHeight }];
                currentY = y;
            }
        }
    ...
    async getCoordinatesFromUrl(url: string, searchWord: string, pageNumber: number) {
        pdfjs.GlobalWorkerOptions.workerSrc = null;
        console.log(url)
    ...
    async stampTest(
        ...
        pageNumber: number,
        fileName: string,
        boundingBoxes: { visLLX: number; visLLY: number; visURX: number; visURY: number },
    ) {
